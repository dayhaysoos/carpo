...
The first implemented tracer bullet registers `getCarpoInstructions`, `readClipWorkspace`, and `proposeClips`. It shares the existing Clip Proposal Review with Think, binds proposals to the current video and a content-derived transcript revision, preserves manual range corrections across idempotent retries, and leaves unsupported browsers on the normal manual interface. Transcript proposals require real overlapping block IDs. An explicit `basis: "timestamps"` instead requires an empty `sourceBlockIds` array, a rationale, and a ready retained source with a known duration. Timestamp selections do not claim transcript or sampled-frame evidence; the review records that provenance and asks the user to preview before approval. Draft revision and broader documentation tools remain future slices.
...
Private workspace tools register only when the relevant authenticated Carpo workspace is available. The public homepage registers only `getCarpoInstructions`, which directs the user to sign in and select a source without reading private state. Tools use the same authorization, validation, idempotency, and provider-independent application boundaries as the manual and Think adapters. Mutating tools return structured validation results and the resulting current proposal state.

`readClipWorkspace` includes the active video's clips, processing failures, and private outputs when encoding is complete. `readCaptionTrack` returns VTT/SRT URLs only for a saved track, and a captioned MP4 only after its render completes. These URLs retain normal session authorization and are not share links. Caption proposals stay unsaved until the user saves them; saving must preserve manual corrections and must not reapply the original agent suggestion when the saved revision changes.
