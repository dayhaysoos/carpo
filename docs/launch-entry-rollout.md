...
Disable Worker version preview URLs with the checked-in Wrangler setting before replacing the Worker-wide destination. Keep cookies HttpOnly and scoped to the whole host, so one sign-in covers `/api`, `/create`, and `/library`.
